...
for i in $(seq 0 "$WORKERS"); do
	worker_node_name="worker-node-${i}"
        sshh -q -o BatchMode=yes -o ConnectTimeout=10 ${VM_USER}@${worker_node_name} /tmp/byo.node | exit && echo $host "$worker_node_name: SSH Connection...OK" || echo $host "$worker_node_name: SSH Connection...FAILED" | exit 1
        shopt -s lastpipe
